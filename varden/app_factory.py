...
def create_app(config: AppConfig) -> FastAPI:
    ...
    def run_demo_scenarios(tenant_id: str):
        scenarios = [
            ...
            {"name": "monitor", "payload": {"type": "tool_call", "tool": "sql.query", "args": {"args": ["select id, email from users limit 1"], "kwargs": {"sql": "select id, email from users limit 1", "database": "app"}}, "agent_name": "monitor-demo-agent", "trace_id": "demo-trace-monitor"}, "raw": {"args": ["select id, email from users limit 1"], "kwargs": {"sql": "select id, email from users limit 1", "database": "app"}}},
        ...
        try:
            from .openai_mod import get_demo_prompt_scenario
            scenarios.append(get_demo_prompt_scenario())
        except Exception as e:
            import logging
            logging.getLogger("varden").warning(f"Skipping OpenAI demo scenario: {e}")
        ...
        for scenario in scenarios:
            ...
            if "forced_decision" in scenario:
                ...
                decision.matched_rule = {"id": "openai-live-mod", "name": "Live API Moderation"}
