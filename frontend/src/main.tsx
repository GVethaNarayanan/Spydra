...
import './styles/spydra-theme.css';
import { SilkGate, SpiderMark } from './components/atmosphere/SilkGate';
import { WebAtmosphere } from './components/atmosphere/WebAtmosphere';
import { SpiderCanvas } from './components/atmosphere/SpiderCanvas';
...
function Shell() {
  const [token, setToken] = usePersistentState<string>('varden.token', 'admin-demo-key');
  // If token was cached as empty from a previous session, reset to demo key
  React.useEffect(() => { if (!token) setToken('admin-demo-key'); }, []);
  ...
  useEffect(() => {
    const onDemo = () => {
      if (!token) return;
      setNotice('Spinning demo strands…');
      (async () => {
        try {
          const payload = await api<any>('/demo/run', { method: 'POST', body: '{}' }, token);
          setOverview(payload.dashboard);
          const firstTrace = payload.dashboard?.recent_traces?.[0]?.trace_id || payload.dashboard?.trace_catalogue?.[0]?.trace_id || '';
          if (firstTrace) setSelectedTraceId(firstTrace);
          setNotice('Demo traces landed — allow, warn, and block');
        } catch (e: any) {
          setError(e?.message || 'Failed to run demo');
        }
      })();
    };
    window.addEventListener('spydra:demo', onDemo);
    return () => window.removeEventListener('spydra:demo', onDemo);
  }, [token]);
    ...
    <div className="shell">
      ...
      <aside className="sidebar">
        <div className="brand">
          <div className="brand__markWrap">
            <div className="brand__icon brand__icon--svg"><SpiderMark /></div>
            <span className="brand__pulse" aria-hidden="true" />
          </div>
          <div className="brand__copy">
            <div className="brand__eyebrow">The watcher</div>
            <div className="brand__title">Spydra</div>
            <div className="brand__subtitle">Runtime web</div>
          </div>
        </div>
        <nav className="nav">
        ...
        <div className="sidebar__footer">
          <button className="button button--ghost" onClick={() => refreshOverview().catch((e:any)=>setError(e?.message||'Refresh failed'))}>🕸️ Refresh snapshot</button>
        </div>
      </aside>
      ...
      <main className="main">
        <header className={classNames('topbar', 'card', (page === 'authority' || page === 'webshield') && 'topbar--compact')}>
          <div>
            <div className="eyebrow">Live silk · operations</div>
            <h1>{page === 'impact' ? 'Which strands are firing' : page === 'rules' ? 'Policy workspace' : page === 'decision' ? 'Decision drilldown' : page === 'coverage' ? 'Holes in the web' : page === 'webshield' ? 'Web Shield' : page === 'authority' ? 'Authority & Provenance' : 'The web, in real time'}</h1>
            <p className="muted">{page === 'impact' ? 'See which rules are carrying the heaviest load across live traffic and drill into who they affect, where they fire, and where false positives may be hiding.' : page === 'coverage' ? 'Observed behaviour with little or no active policy coverage. Surface blind spots, inspect why they are uncovered, and draft the next rule faster.' : page === 'webshield' ? 'Govern WebMCP registrations, invocations, and tool results as untrusted browser input.' : page === 'authority' ? 'Whether the causal chain was authorised to exercise the capability the agent attempted.' : 'Every agent action is a strand. Watch what it attempted, why Spydra scored it, and how policy cut — or kept — the thread.'}</p>
          </div>
...
function App() {
  const [entered, setEntered] = useState(() => {
    try { return sessionStorage.getItem('spydra.entered') === '1'; } catch { return false; }
  });
  ...
  function enter() {
    ...
    enterTimer.current = window.setTimeout(() => {
      try { sessionStorage.setItem('spydra.entered', '1'); } catch {}
      setEntered(true);
      setEntering(false);
    }, 1150);
  }
  ...
  return (
    <>
      {!entered ? <SilkGate entering={entering} onEnter={enter} /> : null}
      <SpiderCanvas />
      <WebAtmosphere
        onSpiderClick={() => {
          if (!entered) { enter(); return; }
          window.dispatchEvent(new CustomEvent('spydra:demo'));
        }}
      />
      <Shell />
    </>
